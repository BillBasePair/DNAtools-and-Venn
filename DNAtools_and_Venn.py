...
with tab4:
    with st.expander("About This Tab"):
        st.write("Visualize overlaps between 2-4 sequence sets from uploaded files, with downloadable 4-set diagrams and selectable intersection sequences for export. Use 'Run Comparison' to analyze exact or fuzzy matches.")
    ...
    if venn_files and 2 <= len(venn_files) <= 4:
        ...
        if num_sets == 2:
            ...
            fig = plt.figure()
            venn2([sets[keys[0]], sets[keys[1]]], set_labels=keys)
            st.pyplot(fig)
        elif num_sets == 3:
            ...
            fig = plt.figure()
            venn3([sets[keys[0]], sets[keys[1]], sets[keys[2]]], set_labels=keys)
            st.pyplot(fig)
        elif num_sets == 4:
